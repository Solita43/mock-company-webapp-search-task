...
public class SearchController {
    ...
    public Collection<ProductItem> search(@RequestParam("query") String query) {
        ...
        // Check if the query is looking for an exact match
        ...
        for (ProductItem item : allItems) {
            ...
            } else if (item.getName().toLowerCase().contains(lowerCaseQuery) || item.getDescription().toLowerCase().contains(lowerCaseQuery)) {
                matchesSearch = true;
            }

            if (matchesSearch) itemList.add(item);
        }
        return itemList;
    }
}
